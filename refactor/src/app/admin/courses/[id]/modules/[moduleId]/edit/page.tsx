...
export default async function EditModulePage({
  ...
  // Find the course title for the breadcrumb
  const courseLink = mod.courseModules.find((cm) => cm.course.id === courseId);
  const courseTitle = courseLink?.course.title ?? "Course";
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        {mod.courseModules.length > 1 && (
          <p className="text-sm text-muted-foreground mt-1">
            Note: this module is shared across {mod.courseModules.length} courses. 
            Changes to the title and description will apply to all of them.
          </p>
        )}
      </div>
